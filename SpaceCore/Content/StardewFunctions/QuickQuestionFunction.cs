...
internal class QuickQuestionFunction : BaseFunction
{
    ...
    public override SourceElement Simplify(FuncCall fcall, ContentEngine ce)
    {
        if (fcall.Parameters.Count != 2)
            throw new ArgumentException($"QuickQuestion function must have exactly two parameters (a quqestion, and a array of answer objects (Response and Commands)), at {fcall.FilePath}:{fcall.Line}:{fcall.Column}");
        ...
        foreach (var respEntry in responsesArray.Contents)
        {
            ...
            {
                throw new ArgumentException($"QuickQuestion response Commands invalid type (must be array or string), at {fcall.FilePath}:{fcall.Line}:{fcall.Column}");
            }
